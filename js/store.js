...
(function (global) {
  ...
  // 工程8種。選択肢はこの順に並べる（CLAUDE.md 3）
  var STAGES = ['初校', '修正', 'ラフ', 'MT', '入稿', '納品', 'TW', '有休'];

  /*
   * 「MT」「入稿」「納品」「TW」「有休」は日付の性質を持つ工程で、
   * まとめて「文字ラベル工程」と呼びます（CLAUDE.md 3）。
   * 常に1日幅で、画面ではバー（背景色）を描かず文字ラベルのみを出す（CLAUDE.md 5.5）。
   */
